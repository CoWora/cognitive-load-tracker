...
try:
    ...
    GW_AVAILABLE = False
    print("pygetwindow 未安装，无法检测活动窗口")

# 浏览器关键词 (包含更多变体)
BROWSER_KEYWORDS = ['edge', 'chrome', 'firefox', 'brave', 'opera', 'luogu', '洛谷', 
                    'microsoft edge', '- edge', 'google chrome', 'mozilla']
# 代码编辑器关键词
...
def is_browser_active() -> bool:
    """检查当前活动窗口是否是浏览器（而非代码编辑器）"""
    title = get_active_window_title().lower()
    
    # 如果标题为空，默认认为是浏览器（避免全部变成代码区）
    if not title:
        return True
    
    # 先检查是否是代码编辑器
    for keyword in CODE_EDITOR_KEYWORDS:
        if keyword in title:
            return False
...
class BlinkDetector:
    ...
    def _init_mediapipe(self):
        """眨眼检测已禁用（MediaPipe 兼容性问题）"""
        # 暂时禁用眨眼检测
        self.available = False
        # print("[Blink] Blink detection disabled (compatibility issue)")
...
class AOIHTTPServer:
    ...
    def _update_page_info(self, data):
        ...
        if not self.connected:
            self.connected = True
            print(f"[HTTP] Browser plugin connected!")
        ...
        if found_count > 0 and not hasattr(self, '_aoi_logged'):
            ...
            print(f"[AOI] Found {found_count} regions:")
            for aoi_id, region in self.page_info.aoi_regions.items():
                print(f"  {aoi_id}: ({region.x1},{region.y1}) - ({region.x2},{region.y2})")
    ...
    def start(self):
        ...
        def run_server():
            ...
            print(f"[HTTP] 服务器启动在 http://{self.host}:{self.port}")
            print(f"[HTTP] 等待浏览器插件连接...")
            while self.running:
    ...
    def get_aoi_at_position(self, screen_x, screen_y) -> Tuple[str, str]:
        """根据屏幕坐标判断 AOI"""
        # 首先检查当前活动窗口是否是浏览器
        # 如果不是浏览器（如 VSCode、Cursor 等），直接返回代码区
        ...
        # 不在洛谷题目页
        if not self.page_info.is_problem_page:
            return "F_CODE_EDITOR", "Code Editor"
        
        # 检查各 AOI 区域
        for aoi_id, region in self.page_info.aoi_regions.items():
            if region.found:
                if region.x1 <= screen_x <= region.x2 and region.y1 <= screen_y <= region.y2:
...
class CognitiveLoadCollector:
    def __init__(self, output_dir="cognitive_data"):
    ...
    def end_task(self, result: str, screen_w: int, screen_h: int):
        ...
        self.add_event("TASK_END", f"{result} | {duration:.1f}s")
        print(f"■ 任务结束: {result} | {duration:.1f}s | 评分: {d}, {e}")
    ...
    def record_gaze(self, screen_x: int, screen_y: int, yaw: float, pitch: float,
        ...
        if self.current_aoi != aoi_id:
            if self.current_aoi is not None and self.current_task:
...
def main():
    ...
    print("\n" + "=" * 70)
    print("Hotkeys:")
    print("  C       - Calibrate gaze")
    print("  +/-     - Adjust smoothing (+ = more smooth, - = less smooth)")
    print("  N       - Start new task")
    print("  1/2/3/4 - Set difficulty")
    print("  A/W/T/G - End task (AC/WA/TLE/GiveUp)")
    ...
    try:
        while True:
            ...
            is_luogu = page_info.is_problem_page
            ...
            win_type = "Browser" if is_browser else "Code"
            win_color = (100, 255, 100) if is_browser else (255, 255, 100)
            ...
            cv2.imshow("Cognitive Load Collector V3", frame)
        ...
        try:
            ...
            print(f"Data saved to: cognitive_data/{collector.session_id}/")
            print(f"  - Gaze records: {len(collector.session_data.gaze_records)}")
            print(f"  - Fixations: {len(collector.session_data.fixations)}")
            print(f"  - Blinks: {len(collector.session_data.blinks)}")
            print(f"  - AOI transitions: {len(collector.session_data.transitions)}")
            print(f"  - Tasks: {len(collector.session_data.tasks)}")
        except Exception as e:
